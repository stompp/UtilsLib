...
{
	return FloatParts::get(value, digits);
	// FloatParts p;
	// p.integer = (long)value;
	// p.decimal = ((powf(10, 1 + digits) * abs(value - p.integer)) + 5) / 10;
	// return p;
}
#ifdef ARDUINO
...
void readStreamLineToString(Stream *p, String &buff, unsigned long timeout, unsigned int maxBytes)
{
	...
	if (p->available())
	{
		...
		bool findLineEnd = true;
		unsigned int readBytes;
		while (findLineEnd && ((millis() - lastRead) < timeout))
		{
